<template>
    <div ref="modalRef" class="modal fade" id="quickViewModal" tabindex="-1" aria-hidden="true">
...
import { initTnsSliders } from "@/Plugins/tns-slider";
...
onMounted(() => {
    // Use global bootstrap object instead of importing Modal
    if (typeof bootstrap !== 'undefined' && modalRef.value) {
        bsModal = new bootstrap.Modal(modalRef.value);
        
        modalRef.value.addEventListener("shown.bs.modal", () => {
            initTnsSliders(); // 🔥 slider works now
        });
    } else {
        console.error('Bootstrap is not loaded globally or modalRef is null');
    }
});
